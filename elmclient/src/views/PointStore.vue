...
<template>
	<div class="wrapper">
		...
		<!-- 用来容纳剩余积分的盒子 -->
		<div class="rest-box">
			<p class="rest-point">剩余积分:&nbsp</p>
			...
			substractPoint(substractNum) {
				...
				for (const availablePoint of this.availablePointArr) {
					...
					if (subNum < availablePoint.pointNum) {
						...
						this.$axios.post(
							`/PointController/updatePointNumById/${availablePoint.pointId}/${availablePoint.pointNum-subNum}`
						);
						subNum = 0;
					}
...
<style scoped>
	...
	.wrapper .coupon-card .purchase-button:hover {
